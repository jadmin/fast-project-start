...
test('tags can be the only result', function (assert) {
  ...
  data.query({
    term: 'templates.ops.test'
  }, function (data) {
    assert.equal(data.results.length, 1);

    var item = data.results[0];

    assert.equal(item.id, 'templates.ops.test');
    assert.equal(item.text, 'templates.ops.test');
  });
});
...
test('tags are injected as options', function (assert) {
  ...
  data.query({
    ...
    assert.equal($tag.val(), 'templates.ops.test');
    assert.equal($tag.text(), 'templates.ops.test');
  });
});
