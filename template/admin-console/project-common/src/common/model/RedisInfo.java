 ...
 * @version $Id: RedisInfo.java ${currentTime} Exp $
 */
public class RedisInfo {
	...
	static {
		...
		map.put("db1", "db1的key的数量,以及带有生存期的key的数,平均存活时间");
		
		map.put("rss_overhead_ratio", "服务内存占用物理内存比例");
		map.put("used_memory_dataset_perc", "数据占用的内存大小的百分比，100%*(used_memory_dataset/(used_memory-used_memory_startup))");
		map.put("io_threads_active", "标志 IO 线程的激活状态，默认为 0 ，也就是非激活态");
		map.put("allocator_rss_ratio", ">1 说明内存碎步很大，<1 说明 内存到了交换内存了，说明：redis显示自己使用的内存大于系统判断该进程使用的内存，说明有内存碎片，否则说明redis使用了交换内存");
		...
		map.put("aof_last_cow_size", "AOF过程中父进程与子进程相比执行了多少修改(包括读缓冲区，写缓冲区，数据修改等)");
		...
		map.put("used_memory_overhead", "Redis维护数据集的内部机制所需的内存开销,包括所有客户端输出缓冲区、查询缓冲区、AOF重写缓冲区和主从复制的backlog");
		map.put("master_replid", "master启动时生成的40位16进制的随机字符串，用来标识master节点");
