...
public class KVCacheServiceImpl implements KVCacheService {

    private RedisTemplate<String, String> redisTemplate;

    private static final Logger log = LoggerFactory.getLogger(KVCacheServiceImpl.class);
	...
	public boolean lock(final String key, final int seconds) {
    	...
    	long startNanoTime = System.nanoTime(); // (ns)当前时间
    	long aquireTimeout = 1*1000*1000000L; // (ns)请求锁超时时间 1秒内拿不到锁，直接返回获取锁失败
    	
    	try {
    		while((System.nanoTime() - startNanoTime) < aquireTimeout) { // 未超时，就一直请求，直到拿到锁
    			if(redisTemplate.opsForValue().setIfAbsent(key, "1")) {
    				redisTemplate.opsForValue().set(key, "1", seconds, TimeUnit.SECONDS); 
    				log.warn("[tryLock]\t key={}\t msg=GET_LOCK_OK", key);
                    return true;  
                } else {
                	log.info("[tryLock]\t key={}\t msg=LOCKED_BY_OTHERS", key);
                }  
                Thread.sleep(Numbers.random(66, 100)); // 随机休眠X毫秒
    		}
		} catch (Exception e) {
			log.error("[tryLock]", e);
		}
